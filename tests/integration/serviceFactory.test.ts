...
describe("serviceFactory integration with ProjectConfigService", () => {
	...
	beforeEach(async () => {
		...
		testDir = path.join(process.cwd(), 'factory-test-' + crypto.randomUUID().slice(0, 8));
	...
	it("should create ProjectConfigService instance in services", () => {
		...
		expect(typeof services.projectConfigService.getProjectConfig).toBe("function");
		expect(typeof services.projectConfigService.setProjectConfig).toBe("function");
	...
	it("should create fresh ProjectConfigService instance after reset", () => {
		...
		resetServices();
		const services2 = getServices();
		expect(services1.projectConfigService).not.toBe(services2.projectConfigService);
	});

	it("should integrate project config with language detection", async () => {
		const services = getServices();
		
		// Test that LanguageConfigService can work with project config
