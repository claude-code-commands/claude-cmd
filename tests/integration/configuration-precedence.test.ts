...
import { getServices, resetServices } from "../../src/services/serviceFactory.js";
...
describe("Configuration Precedence Integration", () => {
	...
	beforeEach(async () => {
		...
		testDir = path.join(process.cwd(), 'test-' + crypto.randomUUID().slice(0, 8));
	...
	afterEach(async () => {
		...
		try {
			...
			const userConfigPath = services.languageConfigService.getConfigPath();
	...
	it("should work correctly when project config file doesn't exist", async () => {
		...
		await services.languageConfigService.setLanguage("fr");
		
		// Project config doesn't exist - should return null
		const projectConfig = await services.projectConfigService.getProjectConfig();
		expect(projectConfig).toBeNull();
		
		// Should fall back to user config
		const effectiveLanguage = await services.languageConfigService.getEffectiveLanguageWithProjectConfig(projectConfig);
	...
	it("should prioritize environment variable over project and user config", async () => {
		...
		await services.projectConfigService.setProjectConfig({ preferredLanguage: "es" });
	...
	it("should fall back to user config when project config has no language preference", async () => {
		...
		await services.projectConfigService.setProjectConfig({ otherSetting: "value" });
	...
	it("should handle project config validation correctly", async () => {
		...
		const retrievedConfig = await services.projectConfigService.getProjectConfig();
		...
		await expect(services.projectConfigService.setProjectConfig(invalidConfig)).rejects.toThrow();
	});
	...
	it("should merge project and user configurations correctly", async () => {
		...
		// Get user config (simulated as it would come from LanguageConfigService)
		const userConfig = { 
			preferredLanguage: await services.languageConfigService.getCurrentLanguage(),
			userSetting: "userValue"
		};
		
		const mergedConfig = services.projectConfigService.mergeConfigs(projectConfig, userConfig);
	...
	it("should support complete precedence chain in realistic scenario", async () => {
		...
		await services.languageConfigService.setLanguage("fr"); // User preference
		await services.projectConfigService.setProjectConfig({ preferredLanguage: "es" }); // Project preference
		...
		const effectiveLanguage2 = await services.languageConfigService.getEffectiveLanguageWithProjectConfig(projectConfig);
		...
		const emptyProjectConfig = await services.projectConfigService.getProjectConfig();
		
		// User config should now win
		const effectiveLanguage3 = await services.languageConfigService.getEffectiveLanguageWithProjectConfig(emptyProjectConfig);
