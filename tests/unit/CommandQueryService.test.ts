...
describe("CommandQueryService", () => {
	...
	beforeEach(() => {
		...
		// Create CommandQueryService with in-memory dependencies
	...
	describe("listCommands", () => {
		it("should return cached commands when cache hit occurs", async () => {
			...
			// Execute
			const result = await commandQueryService.listCommands({ language: "en" });
		...
		it("should refresh from repository when cache is expired", async () => {
			...
			const result = await commandQueryService.listCommands({
				language: "en",
		...
		it("should handle repository errors gracefully", async () => {
			...
			await expect(
				commandQueryService.listCommands({ language: "xx" }),
	...
	describe("searchCommands", () => {
		it("should filter commands by query in name and description", async () => {
			...
			const result = await commandQueryService.searchCommands("debug", {
				language: "en",
			});
		...
		it("should return empty array when no commands match query", async () => {
			...
			const result = await commandQueryService.searchCommands(
				"nonexistentxyz",
				{
					language: "en",
				},
		...
		it("should perform case-insensitive search", async () => {
			...
			const lowerResult = await commandQueryService.searchCommands("debug", {
				language: "en",
			});
			const upperResult = await commandQueryService.searchCommands("DEBUG", {
				language: "en",
			});
	...
	describe("getCommandInfo", () => {
		it("should return command metadata when command exists", async () => {
			// Execute
			const result = await commandQueryService.getCommandInfo("debug-help", {
				language: "en",
			});
		...
		it("should throw CommandNotFoundError when command does not exist", async () => {
			...
			await expect(
				commandQueryService.getCommandInfo("nonexistent-command", {
					language: "en",
				}),
