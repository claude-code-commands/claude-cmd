...
describe("Repository Interface", () => {
	...
	describe("getManifest", () => {
		test("should retrieve manifest for English language", async () => {
			...
			const debugCommand = manifest.commands.find(cmd => cmd.name === "debug-help");
		...
		test("should handle different allowed-tools formats", async () => {
			...
			// String format
			const frontendCommand = manifest.commands.find(cmd => cmd.name === "frontend:component");
			expect(typeof frontendCommand?.["allowed-tools"]).toBe("string");
		});

		test("should throw ManifestError for unsupported language", async () => {
			await expect(repository.getManifest("invalid-lang")).rejects.toThrow(ManifestError);
			await expect(repository.getManifest("invalid-lang")).rejects.toThrow("Language not supported");
		});

		test("should throw ManifestError for network errors", async () => {
			await expect(repository.getManifest("network-error")).rejects.toThrow(ManifestError);
			await expect(repository.getManifest("network-error")).rejects.toThrow("Network connection failed");
		});

		test("should throw ManifestError for timeout errors", async () => {
			await expect(repository.getManifest("timeout")).rejects.toThrow(ManifestError);
			await expect(repository.getManifest("timeout")).rejects.toThrow("Request timed out");
		});
		...
		test("should track request history", async () => {
			const repo = repository as InMemoryRepository;
			repo.clearRequestHistory();
			
			await repository.getManifest("en");
			const history = repo.getRequestHistory();
	...
	describe("getCommand", () => {
		...
		test("should throw CommandNotFoundError for non-existent command", async () => {
			await expect(repository.getCommand("non-existent", "en")).rejects.toThrow(CommandNotFoundError);
		...
		test("should throw CommandNotFoundError for command not in manifest", async () => {
			...
			await expect(repository.getCommand("test-gen", "fr")).rejects.toThrow(CommandNotFoundError);
		});

		test("should throw CommandContentError for content retrieval failures", async () => {
			await expect(repository.getCommand("content-error", "en")).rejects.toThrow(CommandContentError);
		...
		test("should throw CommandContentError for missing files", async () => {
			await expect(repository.getCommand("missing-file", "fr")).rejects.toThrow(CommandContentError);
			await expect(repository.getCommand("missing-file", "fr")).rejects.toThrow("File not found on server");
		});
		...
		test("should verify command exists in manifest before fetching content", async () => {
			...
			await expect(repository.getCommand("debug-help", "en")).resolves.toBeDefined();
			
			// This should fail - command not in manifest
			await expect(repository.getCommand("non-existent-command", "en")).rejects.toThrow(CommandNotFoundError);
		});
	});
	...
	describe("language parameter threading", () => {
		...
		test("should validate command existence per language", async () => {
			...
			await expect(repository.getCommand("test-gen", "en")).resolves.toBeDefined();
			await expect(repository.getCommand("test-gen", "fr")).rejects.toThrow(CommandNotFoundError);
		});
	});
	...
	describe("options parameter handling", () => {
		test("should accept undefined options", async () => {
			await expect(repository.getManifest("en", undefined)).resolves.toBeDefined();
			await expect(repository.getCommand("debug-help", "en", undefined)).resolves.toBeDefined();
		});

		test("should accept empty options object", async () => {
			const options: RepositoryOptions = {};
			await expect(repository.getManifest("en", options)).resolves.toBeDefined();
			await expect(repository.getCommand("debug-help", "en", options)).resolves.toBeDefined();
		});
		...
		test("should track options in request history", async () => {
			...
			const options: RepositoryOptions = { forceRefresh: true, maxAge: 3600000 };
